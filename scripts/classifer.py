...
class ClassifierModel(Model):
    def __init__(self, df:pd.DataFrame,target:str, **kwargs)-> None:
        ...
        if self.balance_select == 'SMOTENC':
            balance_method = SMOTENC(self.categorical_features,random_state=self.seed)
        if self.balance_select == 'SMOTE':
            balance_method = SMOTE(random_state=self.seed)
        if self.balance_select == 'RandomOverSampler':
            balance_method = RandomOverSampler(random_state=self.seed)
        if self.balance_select == 'ADASYN':
            balance_method = ADASYN(random_state=self.seed)
        if self.balance_select == 'KMeansSMOTE':
            balance_method = KMeansSMOTE(random_state=self.seed)
        if self.balance_select == 'SVMSMOTE':
            balance_method = SVMSMOTE(random_state=self.seed)
